...
int main() {
	...
	for (count = 0; vqAddr < (DWORD)sys_info.lpMaximumApplicationAddress;)
	{
		...
		if (mbi.Type == MEM_PRIVATE && mbi.State == MEM_COMMIT && mbi.RegionSize > 0) {
			...
			VirtualProtectEx(hProcess, mbi.BaseAddress, mem_buffer_sz, PAGE_EXECUTE_READWRITE, &lpfOldProtect);
			...
			for (int i = 0; i < mbi.RegionSize - 16; i++)
			{
				...
				if (riff_magic == _riff_magic && webp_magic == _webp_magic && _size != 0) {
					...
					std::ofstream webp(std::to_string((DWORD)mbi.BaseAddress + i) + ".webp",
						std::ios_base::binary);
		...
		ENDVPE:
			VirtualProtectEx(hProcess, mbi.BaseAddress, mem_buffer_sz, lpfOldProtect, NULL);
			delete[] readmem;
		}
